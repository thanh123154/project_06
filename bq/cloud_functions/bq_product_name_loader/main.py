...
def _load_schema() -> list[bigquery.SchemaField] | None:
    ...
    try:
        with open(SCHEMA_PATH, "r", encoding="utf-8") as f:
            data = json.load(f)
        # Support either list or {"fields": [...]} object
        schema_list = data.get("fields") if isinstance(data, dict) else data
...
def trigger_bigquery_load(event, context):
    ...
    Calls bq_product_name_loader function.
    """
    return bq_product_name_loader(event, context)


def bq_product_name_loader(event, context):
    """
...
if __name__ == "__main__":
    ...
    test_event = {
        ...
        "name": "exports/product_name/test.jsonl",
    ...
    result = bq_product_name_loader(test_event, test_context)
